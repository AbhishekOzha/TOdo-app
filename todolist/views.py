...
class TodayTodoItemList(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return TodoItem.objects.filter(Q(due_date=today) | Q(due_date__isnull=True)).order_by('created_at')
...
class UpcomingTodoItemList(generics.ListAPIView):
    ...
    def get_queryset(self):
        today = timezone.localdate()
        return TodoItem.objects.filter(Q(due_date__gt=today) | Q(due_date__isnull=False)).order_by('due_date')